...
@section('title', 'Create Partnership Way')
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card card-primary">
                        ...
                        <form action="{{ route('partner_ways.store') }}" id="partnership_way_create" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="card-body">
                                <!-- Title -->
                                <div class="form-group">
                                    <label for="title">Way Title</label>
                                    <input type="text" class="form-control @error('title') is-invalid @enderror" name="title" id="title" value="{{ old('title') }}" placeholder="Enter way title">
                                    @error('title')
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="url" class="form-control @error('link') is-invalid @enderror" name="link" id="link" value="{{ old('link') }}" placeholder="Enter URL (e.g. https://example.com)">
                                    @error('link')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        // jQuery Validation
        $("#partnership_way_create").validate({
            rules: {
                title: {
                    required: true,
                    minlength: 3,
        ...
        $('#resetBtn').click(function () {
            $('#partnership_way_create')[0].reset();
            if (editorInstance) {
