...
@section('content')
                    ...
                    <div class="left-side-deatils">
                        ...
                        <div class="info-desc">
                            ...
                            <div class="horse-info-box">
                                ...
                                <div class="horse-info-row"><span class="horse-label">Stalls Available For Hauling or Show
                                        :</span> {{ @$products->productDetail->stalls_available_haulings }}</div>

                                <div class="horse-info-row"><span class="horse-label">Date Available From :</span>
                                    {{ @$products->productDetail->fromdate }}</div>
                                <div class="horse-info-row"><span class="horse-label">Date Available To :</span>
                                    {{ @$products->productDetail->todate }}</div>
                                <div class="horse-info-row"><span class="horse-label">Salary :</span>
                                    {{ @$products->productDetail->salary }}</div>

                                <div class="horse-info-row"><span class="horse-label">Hourly Pay :</span>
                                    {{ @$products->productDetail->hourly_price }}</div>
                                <div class="horse-info-row"><span class="horse-label">Fixed Pay :</span>
                                    {{ @$products->productDetail->fixed_price }}</div>

                                <div class="horse-info-row"><span class="horse-label">Haulings: From Location :</span>
                                    {{ @$products->productDetail->haulings_location_from }}</div>
                                <div class="horse-info-row"><span class="horse-label">Haulings: To Location :</span>
                                    {{ @$products->productDetail->haulings_location_to }}</div>

                                <div class="horse-info-row"><span class="horse-label">Precise Location :</span>
                                    {{ @$products->productDetail->precise_location }}</div>
                                <div class="horse-info-row"><span class="horse-label">Street:</span>
                                    {{ @$products->productDetail->street }}</div>

                                <div class="horse-info-row"><span class="horse-label">City:</span>
                                    {{ @$products->productDetail->city }}</div>
                                ...
                                <div class="header-popup mb-3">
                                    <div>
                                        ...
                                        <select id="monthSelect"
                                            class="form-select custom-dropdown w-auto d-inline-block">
                                            <!-- Populated by JS -->
                                        </select>
                                    </div>
                                    <div class="d-flex align-items-center gap-2">
                                        <button id="prevWeek" class="custom-dropdown bg-white px-3"><img
                                                src="{{ asset('front/home/assets/images/icons/arrow-left.svg') }}"
                                                alt="" width="24" /></button>
                                        ...
                                        <button id="nextWeek" class="custom-dropdown bg-white px-3"><img
                                                src="{{ asset('front/home/assets/images/icons/arrow-right.svg') }}"
                                                alt="" width="24" /></button>
                                    </div>
                                </div>
                        ...
                        <div class="card-boxleft">
                            ...
                            @if (@$products->sale_method == 'auction' && @$products->product_status == 'live' && !empty(@$products->bid_expire_date))
                                <div class="pb-3 gap-2">
                                    ...
                                    <h2 class="fw-bold">
                                        {{ '$' . number_format(optional($products->highestBid)->amount ?? $products->productDetail->bid_min_price) }}
                                    </h2>
                                </div>
                            @endif
                            ...
                            @if (
                                ...
                                <div class="pb-3 gap-2">
                                    ...
                                    <h2 class="fw-bold">{{ $products->currency . ' ' . number_format($products->price, 2) }}
                                    </h2>
                                </div>
                            @endif
                                ...
                                <div class="btn-connected gap-2">
                                    ...
                                    @else
                                        ...
                                        @elseif(@$products->sale_method == 'standard' && @$products->transaction_method == 'platform')
                                            @if (@$products->product_status == 'live')
                                                @if (auth()->check())
                                                    <button class="call-price w-100"data-bs-toggle="modal"
                                                        data-bs-target="#exampleModal">Book Now</button>
                                                @else
                                                    <button class="call-price w-100"
                                                        onclick="showLoginModal('You must be logged in to book this service.')">
                                                        Book Now
                                                    </button>
                                                @endif
    ...
    <script>
        ...
        function initializeMap(latitude, longitude, trailLat, trailLong) {
            ...
            var markerIconUrlTrail = "{{ env('MAP_PUBLIC') }}/images/Services Jobs Red.png";
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function populateMonthOptions() {
                ...
                monthList.forEach(monthKey => {
                    ...
                    option.text = date.toLocaleString('default', {
                        month: 'long',
                        year: 'numeric'
                    });
                    monthSelect.appendChild(option);
      ...
      `;
                        ...
                        slots.forEach(([from, to]) => {
                            ...
                            if (isBooked) {
                                ...
                                slotDiv.addEventListener('click', () => {
                                    document.querySelectorAll('.slot.selected-slot')
                                        .forEach(el => {
                ...
                const start = new Date(week[0]).toLocaleDateString('en-GB', {
                    day: 'numeric',
                    month: 'short'
                });
                const end = new Date(week[week.length - 1]).toLocaleDateString('en-GB', {
                    day: 'numeric',
                    month: 'short'
                });
                dateRange.textContent = `${start} - ${end}`;
