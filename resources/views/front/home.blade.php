                        ...
                        <div class="banner_area_inner_search">
                            <div class="form_bx">
                                <img src="{{ asset('front/home/assets/images/icons/search.svg') }}" alt="search-icon"
                                    class="icon" />
                                <input type="text" placeholder="Search for" name="search" id="search"
                                    autocomplete="off" />
                            </div>
                            <div class="form_bx">
                                <img src="{{ asset('front/home/assets/images/icons/search.svg') }}" alt="search-icon"
                                    class="icon" />
                                <input type="text" autocomplete="off" placeholder="Located in"
                                    value="{{ request()->query('location') }}"
                                    onkeyup 
                        ="initializeLocationAutocomplete()" name="location"
                                    id="location" />
                        ...
                        <div class="links">
                            @if ($ios_app = social_links('ios_app'))
                                <a href="{{ $ios_app }}" target="_blank" rel="noopener noreferrer">
                                    <img src="{{ asset('front/home/assets/images/vactors/app_store.svg') }}"
                                        alt="" />
                                </a>
                            @endif
                            ...
                            @if ($android_app = social_links('android_app'))
                                <a href="{{ $android_app }}" target="_blank" rel="noopener noreferrer">
                                    <img src="{{ asset('front/home/assets/images/vactors/paly_store.svg') }}"
                                        alt="" />
                                </a>
                            @endif
                        </div>
                ...
                <form action="{{ route('home') }}" id="universalSearchFormAgain">
                    <div class="banner_area_inner_search">
                        <div class="form_bx">
                            <img src="{{ asset('front/home/assets/images/icons/search.svg') }}" alt="search-icon"
                                class="icon" />
                            <input type="text" placeholder="Search for product name" name="search" id="searchName"
                                autocomplete="off" />
                        </div>
                        <div class="form_bx wow zoomIn location-manage-add" data-wow-delay="0.6s">
                            <img src="{{ asset('front/home/assets/images/icons/search.svg') }}" alt="search-icon"
                                class="icon" />
                            <input type="text" autocomplete="off" placeholder="Located in" name="location"
                                id="map-location" value="{{ request()->query('location') }}"
                                onkeyup 
                        ...
                        <div class="form_bx1">
                            <button type="button" class="commen_btn" id="universalSearchButtonAgain">Search</button>
                        </div>
                    </div>
                </form>
            </div>
                ...
                <div class="row">
                    <div class="col-lg-6 my-auto">
                        <div class="about_area_inner">
                            ...
                            <a href="{{ url('/aboutus') }}"><span>Read More</span><i
        ...
        <section class="forbuyer_area">
            <div class="container z-11">
                ...
                <div class="row mt-5 align-items-center">
                    <div class="col-lg-6">
                        <img class="buyers_and_browsers" src="{{ asset('storage/' . $buyerBrowserData->image) }}"
                            alt="" />
                    </div>
                    <div class="col-lg-6">
            ...
            <div class="container">
                <div class="row">
                    <div class="col-lg-12 d-flex justify-content-between align-items-center flex-wrap ">
                        ...
                        <button class="commen_btn border-0" onclick="window.location.href='{{ route('hj-forum') }}'">See
                            All Posts</button>
                    </div>
                    ...
                    <div thumbsSlider="" class="swiper mySwiper1">
                        <div class="swiper-wrapper">
                            @foreach (@$hjForumData as $key => $forum)
                                <div class="swiper-slide">
                                    <div class="forum-box">
                                        <img src="{{ asset('/storage/' . @$forum->image) }}" class="image"
                                            alt="logo" width="110px" height="80px">
                    ...
                    <div class="industry_slider_controal justify-content-center mt-4">
                        <button id="slider-prev1" tabindex="0" aria-label="Previous slide"
                            aria-controls="swiper-wrapper-212c6328bee8fed5">
                            <i class="fa-solid fa-arrow-left-long"></i>
                        </button>
                        <button id="slider-next1" tabindex="0" aria-label="Next slide"
                            aria-controls="swiper-wrapper-212c6328bee8fed5">
                            <i class="fa-solid fa-arrow-right-long"></i>
                        </button>
                    </div>
    ...
    <script>
        ...
        window.onload = () => {
            ...
            document.querySelector('.category-scroll').addEventListener('click', function(e) {
                ...
                document.querySelectorAll('.category-scroll button[data-id]').forEach(b => b.classList.remove(
                    'active'));
                ...
                document.querySelectorAll('.red-circle-marker').forEach(el => el.classList.remove(
                    'active-marker'));
        ...
        function fetchEventsByCategory(categoryId) {
            ...
            let searchLatitude=$("#map-latitude").val();
            let searchLongitude=$("#map-longitude").val();
            let searchDate=$("#date").val();
            let searchCategory=$("#searchCategory").val() ;
            searchCategory=searchCategory != "" ? searchCategory :1; 

            // let apiUrl = '{{ url('/api/v1/product/mapbox-list?category=') }}' + categoryId;
            let apiUrl = '{{ url('/mapboxevent') }}' + '?category=' + encodeURIComponent(searchCategory) +  '&name=' + encodeURIComponent(searchName) + '&latitude=' + encodeURIComponent(searchLatitude) +  '&longitude=' + encodeURIComponent(searchLongitude) + '&date=' + encodeURIComponent(searchDate);
            fetch(apiUrl)
                .then(response => {
        ...
        function initializeLocationAutocomplete() {
            ...
            function clearSuggestions() {
                ...
                mapLocationMessage.style.display = 'none';
                selectedIndex = -1;
                suggestions = [];
            }
                        ...
                        .then(data => {
                            ...
                            if (suggestions.length > 0) {
                                suggestions.forEach((suggestion, i) => {
                                    ...
                                    li.addEventListener('click', () => selectSuggestion(i));
                                    mapLocationList.appendChild(li);
                                });
                                mapLocationList.style.display = 'block';
                                locationMessage.style.display = 'none';
                                selectedIndex = -1;
                            } else {
                                locationMessage.style.display = 'block';
            ...
            // Keyboard navigation
            mapLocationInput.addEventListener('keydown', function(e) {
                const items = mapLocationList.querySelectorAll('li');
                if (mapLocationList.style.display === 'block' && items.length > 0) {
                    if (e.key === 'ArrowDown') {
                        e.preventDefault();
                        selectedIndex = (selectedIndex + 1) % items.length;
                        highlightSuggestion(selectedIndex);
        ...
        $(document).ready(function() {
            ...
            $("#universalSearchButtonAgain").click(function(){
                fetchEventsByCategory(1)
            })
        });
            ...
            buttons.forEach(button => {
                button.addEventListener("click", function() {
                    ...
                    const targetContent = document.querySelector(
                        `.toggle-content[data-index='${index}']`);
                    if (targetContent) {
