@foreach($data as $key=>$comment)
    <div class="comment-item">
        ...
        <div class="reply-form mt-2" style="display: none;">
            ...
            @if(auth()->check()) 
                <form action="{{route('productComment')}}" method="post" class="product-comment-reply-form">   
                    @csrf
                    <input type="hidden" name="product_id" value="{{$productId}}">
                    <input type="hidden" name="product_comment_id" value="{{$comment->id}}">
                    <div>
                        <textarea class="form-control mb-2" rows="2" required name="comment" id="comment" placeholder="Write a reply..."></textarea>
                    </div>
                    <button type="submit" class="btn-theme-bg product-comment-reply-form-submit">Post Reply</button>
                </form>
            @else
                <form action="{{route('productComment')}}" method="post" class="product-comment-reply-form-guest">   
                    @csrf
                    <input type="hidden" name="product_id" value="{{$productId}}">
                    <input type="hidden" name="product_comment_id" value="{{$comment->id}}">
                    <div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('.product-comment-reply-form').each(function () {
            $(this).validate({
                rules: {
                    comment: {
                        required: true,
                ...
                submitHandler: function (form) {
                    let $form = $(form);
                    let $submitBtn = $form.find('.product-comment-reply-form-submit');
        ...
        $('.product-comment-reply-form-guest').each(function () {
            $(this).validate({
                rules: {
                    name: {
                        required: true,
    ...
    function addProductCommentReply(page)
    {
