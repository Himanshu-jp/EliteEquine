    ...
    <form action="{{route('product')}}" method="POST" enctype="multipart/form-data" id="productForm">
                ...
                <div class="mt-3">
                    <div class="mb-3">
                        ...
                        <input type="text" autocomplete="off" class="inner-form form-control mb-0" id="title" name="title"
                            value="{{old('title', @$products->title)}}" placeholder="Short and Sweet Name only is Preferred">
                        @if($errors->has('title'))
                                ...
                                @foreach (@$products->image as $key=>$image)
                                    <div class="position-relative">
                                        <a href="{{ asset('storage/' . $image->image) }}" target="_blank">
                                            <img src="{{ asset('storage/' . $image->image) }}" alt="Image" class="img-thumbnail" style="width: 150px;">
                                        </a>
                                        @if(@$products->image->count()>1)
                                            <a href="{{route('removeImage',$image->id)}}"><span class="close-icon">&times;</span></a>
                                        @endif
                                    </div>
                                @endforeach
                            </div>
            ...
            <div class="col-lg-6 mt-3">
                    ...
                    <div class="row">
                        ...
                        @if(@$products->category_id)
                            <div class="position-relative mb-3">                                
                                ...
                                <input type="text" autocomplete="off" class="inner-form form-control mb-0 pe-5" readonly value="{{ @$products->category->name}}" id="category" data-category-id="{{@$products->category->id}}" placeholder="Category">
                            </div>
                        @else
                            <div class="position-relative">
                                <label for="exampleFormControlInput1" class="form-label">Category*</label>
                                <select class="form-select pe-5 mb-2 inner-form form-control" name="category" id="category" >
                                    <option value="">-- Select Category --</option>
                                    @foreach (__categoryData() as $key=>$value)
                                        <option value="{{$key}}" {{(old('category',@$products->category_id)==$key)?'selected':''}}>{{$value}}</option>    
                                    @endforeach
                                </select>            
                                <i class="fi fi-rr-angle-small-down" style="position: absolute;
                                            top: 63%;
                ...
                <div class="mb-3">
                    ...
                    <div class="file-upload">
                        ...
                        <input type="file" id="video" name="video[]" multiple style="display: none; cursor: pointer;"
                            onchange="handleVideoUpload(event)" accept=".mp4,.avi,.mov,.wmv">
                            ...
                            @foreach (@$products->video as $image)
                                <div class="position-relative">
                                    <a href={{asset('storage/'.$image->video_url)}} target="_blank">
                                        <img src="{{asset('front/auth/assets/img/icons/video.svg')}}" class="user-img" alt="" id="editVideo" style="width: 50px; height: 50px;">
                                    </a>
                                    @if(@$products->video->count()>0)
                                        <a href="{{route('removeVideo',$image->id)}}"><span class="close-icon">&times;</span></a>
                                    @endif
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="file-upload">
                            <div class="profile mt-2">
                            <img src="{{(@$products->document)?asset('storage/'.@$products->document):asset('front/auth/assets/img/icons/pdf-icon.svg')}}" class="user-img" alt="" id="editDocument">
                        </div>
                        <input type="file" id="document" name="document[]" multiple style="display: none; cursor: pointer;" onchange="handleDocumentUpload(event)" accept=".pdf,.doc,.docx">
                            ...
                            @foreach (@$products->document as $image)
                                <div class="position-relative">
                                    <a href={{asset('storage/'.$image->file)}} target="_blank">
                                        <img src="{{asset('front/auth/assets/img/icons/pdf-icon.svg')}}" class="user-img" alt="" id="editImg" style="width: 50px; height: 50px;">
                                    </a>
                                    @if(@$products->document->count()>0)
                                    <a href="{{route('removeDocument',$image->id)}}"><span class="close-icon">&times;</span></a>
                                    @endif
                                </div>
                            @endforeach
                        </div>
        ...
        <div class="col-lg-6 mt-3">
            <div class="d-flex align-items-center justify-content-start gap-2 flex-wrap">
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="sale_method" id="standard" value="standard" {{(old('sale_method')=='standard' || @$products->sale_method=='standard')?'checked':''}}>
                    <label class="form-check-label fw-bold text-dark" for="standard">Standard</label>
                </div>
                <div class="form-check" id="auction_action">
                    <input class="form-check-input" type="radio" name="sale_method" id="auction" value="auction" {{(old('sale_method')=='auction' || @$products->sale_method=='auction')?'checked':''}}>
            ...
            <div class="d-flex align-items-center justify-content-start gap-2 flex-wrap">
                ...
                <div class="form-check">
                    ...
                    <label class="form-check-label fw-bold text-dark" for="buyertoseller">Buyer To Seller Connection Only 
                        <span data-toggle="tooltip" data-placement="top" title="Allows direct contact between buyer and seller for this transaction method.">
                            <img src="{{asset('images/letter-i.png')}}" width="15" height="15" alt="" />
                        </span>
                    </label>
                </div>
