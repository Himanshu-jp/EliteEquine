...
const socket = io("https://circlequay.com:3115/");
            ...
            $.ajax({
	            ...
	            success:function(response){
	                if(response.status){
	                	...
	                	// $('.booking_id').html(`
						// 	<div class="usr-ms-img">
						// 		<img src="{{url('/')}}/${response.data.userdata.profile}" alt="">
						// 	</div>
						// 	<div class="usr-mg-info user_name_filed">
						// 		<h3>${response.data.userdata.first_name}<span>${response.data.userdata.role}</span></h3>
						// 		<p>${(parseInt(response.data.userdata.is_online) == 0)? 'Offline':'Online'}</p>
						// 	</div>
						// `);
   ...
   function timeAgo(time_ago) {
        ...
        if(result.status==true)
        {
            ...
            if(data.file_type=='IMAGE')
              ...
              if(chat_type == 'SINGLE'){
		            ...
		            html1 =  `<div class="main-message-box ta-right">
							<div class="message-dt">
									<div class="message-inner-dt">
										<img  id="singledelete" data-msgid="${data.id}" src="{{asset('frontend/bussiness/images/delet00000000452.svg')}}" alt="">
										<p class="chat-emoji">${file}${message}</p>
            ...
            if(data.type=="SINGLE")
            {
              if(data.chatuser[0].get_user && data.chatuser[1].get_user)
              {
                ...
                if(data.chatuser[0].get_user && parseInt(data.chatuser[0].user_id) != parseInt(SENDER_ID))
                {
                  ...
                  profile = data.chatuser[0].get_user.profile_image ? data.chatuser[0].get_user.profile_image: 'https://stage.tasksplan.com/lastMntapp/public/users/images/169502909053.png';  
                ...
                if(data.chatuser[1].get_user &&  parseInt(data.chatuser[1].user_id) != parseInt(SENDER_ID))
                {
                  ...
                  profile = data.chatuser[1].get_user.profile_image ? data.chatuser[1].get_user.profile_image: 'https://stage.tasksplan.com/lastMntapp/public/users/images/169502909053.png';  
    ...
    if(firstthread){
	    ...
	    $('.booking_id').html(`
			...
			<div class="usr-mg-info user_name_filed">
				<h3>${user_name}<span>${user_role}</span></h3>
				<p>${is_online}</p>
			</div>
		`);
  ...
  $(document).on('click', '.leavenow', function(e) {
    ...
    //       type:'POST',
    //       dataType:'json',
    //       url:"{{ route('customer.userunblock') }}",
    //       data:{
    //           roomId:roomId,
    //           _token:'{{ csrf_token() }}'
    //       },
	...
	$(document).on('click','.deletechat',function(){
	    ROOM_ID = $('.thread_details').attr('data-room_id');
